    ...
    : IMvxIoCProvider
{
    ...
    private sealed class ConstructingOpenGenericResolver : IResolver
    {
        ...
        public object Resolve()
        {
            ...
            {
                throw new Exception($"No Generic Type Parameters provided for Type: {_genericTypeDefinition.FullName}");
            }
    ...
    public void RegisterType(Type t, Func<object?> constructor)
    {
        var resolver = new FuncConstructingResolver(() =>
        {
            ...
            {
                throw new Exception($"Constructor failed to return a compatibly object for type {t.FullName}");
            }
    ...
    public object IoCConstruct(Type type, params object?[] arguments)
    {
        ...
        {
            throw new Exception("Failed to find constructor for type {type.FullName} with arguments: {arguments.Select(x => x?.GetType().Name)}");
        }
    ...
    public object IoCConstruct(Type type, IDictionary<string, object>? arguments)
    {
        ...
        if (selectedConstructor == null)
        {
            throw new Exception($"Failed to find constructor for type {type.FullName}");
        }
    ...
    private bool InternalTryResolve(Type type, IResolver resolver, out object? resolved)
    {
        ...
        try
        {
            ...
            {
                throw new Exception("Resolver returned object type {raw.GetType().FullName} which does not support interface {type.FullName}");
            }
    ...
    private bool ShouldDetectCircularReferencesFor(IResolver resolver)
    {
        switch (resolver.ResolveType)
        {
            ...
            case ResolverType.Unknown:
                throw new Exception($"A resolver must have a known type - error in {resolver.GetType().Name}");
            default:
    ...
    private bool TryResolveParameter(Type type, ParameterInfo parameterInfo, out object? parameterValue)
    {
        if (!TryResolve(parameterInfo.ParameterType, out parameterValue))
        {
            ...
            {
                throw new Exception(
                    $"Failed to resolve parameter for parameter {parameterInfo.Name} of type {parameterInfo.ParameterType.Name} when creating {type.FullName}. You may pass it as an argument");
            }
        }
